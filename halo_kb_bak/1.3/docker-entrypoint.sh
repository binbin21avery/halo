...
docker_setup_db() {
	...
	dbAlreadyExists="$(
		POSTGRES_DB= docker_process_sql --dbname halo0root --set db="$POSTGRES_DB" --tuples-only <<-'EOSQL'
			SELECT 1 FROM pg_database WHERE datname = :'db' ;
		EOSQL
	)"
	if [ -z "$dbAlreadyExists" ]; then
		POSTGRES_DB= docker_process_sql --dbname halo0root --set db="$POSTGRES_DB" <<-'EOSQL'
			CREATE DATABASE :"db" ;
		EOSQL
		printf '\n'
	fi

	psql -d $POSTGRES_DB -c "create extension aux_oracle cascade;"
...
docker_setup_env() {
	file_env 'HALO_PASSWORD'

	file_env 'HALO_USER' 'halo'
	file_env 'POSTGRES_DB' "halo"
...
pg_setup_hba_conf() {
	...
	: "${POSTGRES_HOST_AUTH_METHOD:="md5"}"
	{
		...
		echo "host    all             all             0/0                 $POSTGRES_HOST_AUTH_METHOD"
	} >> "$PGDATA/pg_hba.conf"
}
...
halo_postgres_conf(){
	echo "修改配置文件"
	halo_data=/var/lib/postgresql/data
	
	pg_conf=${halo_data}/postgresql.conf
	
	pg_hba_conf=${halo_data}/pg_hba.conf

	echo "修改postgresql.conf"
	sed -i "s/#listen_addresses = 'localhost'/listen_addresses = '*'/" $pg_conf
	sed -i "s/#port = 1921/port = 1921/" $pg_conf
	sed -i "s/max_connections = 100/max_connections = 1000/" $pg_conf
	...
	sed -i "s/max_wal_size = 1GB/max_wal_size = 8GB/" $pg_conf
	sed -i "s/min_wal_size = 80MB/min_wal_size = 2GB/" $pg_conf
	sed -i "s/#default_statistics_target = 100/default_statistics_target = 100/" $pg_conf
	sed -i "s/#log_destination = 'stderr'/log_destination = 'csvlog'/" $pg_conf
	sed -i "s/#logging_collector = off/logging_collector = on/" $pg_conf
	sed -i "s/#random_page_cost = 4.0/random_page_cost = 1.1/" $pg_conf
	sed -i "s/#maintenance_io_concurrency = 10/maintenance_io_concurrency = 200/" $pg_conf
	sed -i "s/#wal_log_hints = off/wal_log_hints = on/" $pg_conf
	...
	sed -i "s/#archive_command = ''/archive_command = 'test ! -f \/var\/lib\/postgresql\/data\/archivedir\/%f \&\& cp %p \/var\/lib\/postgresql\/data\/archivedir\/%f'/" $pg_conf
	sed -i "s/#restore_command = ''/restore_command = 'cp \/var\/lib\/postgresql\/data\/archivedir\/%f %p'/" $pg_conf
	...
	sed -i "s/shared_buffers = 128MB/shared_buffers = $MEM_S/" $pg_conf
	sed -i "s/#effective_cache_size = 4GB/effective_cache_size = $MEM_E/" $pg_conf
	sed -i "s/#max_worker_processes = 8/max_worker_processes = $CPU/" $pg_conf
	sed -i "s/#max_parallel_workers = 8/max_parallel_workers = $CPU/" $pg_conf
	sed -i "s/#max_parallel_workers_per_gather = 2/max_parallel_workers_per_gather = 4/" $pg_conf
	sed -i "s/#max_parallel_maintenance_workers = 2/max_parallel_maintenance_workers = 4/" $pg_conf
	

	echo "修改pg_hba.conf文件"
	echo 'host    all             all             0/0                 md5' >> $pg_hba_conf
