...
docker_setup_db() {
    ...
    psql -c "CREATE USER replica PASSWORD 'halo@123456' REPLICATION;"

    psql -c "CREATE USER patroni SUPERUSER PASSWORD 'patroni';"




	psql -d $POSTGRES_DB -c "create extension pg_stat_statements;"
...
docker_setup_hba_conf() {

	{
					...
					echo "host 		replication 		replica 		0/0 			md5"
			} >> "$PGDATA/pg_hba.conf"
